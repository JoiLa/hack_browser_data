...
type Browser interface {
	...
	// GetName return BrowserStruct name
	GetName() string

	// GetSecretKey return BrowserStruct secret key
	GetSecretKey() []byte
...
var (
	...
	errBrowserNotSupported = errors.New("BrowserStruct not supported")
...
// NewChromium return Chromium BrowserStruct interface
...
// GetAllItems return all chromium items from BrowserStruct
...
// NewFirefox return firefox BrowserStruct interface
...
// PickBrowser return a list of BrowserStruct interface
...
// PickOneBrowser return a list of BrowserStruct interface
func PickOneBrowser(name string) (Browser, error) {
	name = strings.ToLower(name)
...
// PickCustomBrowser pick single BrowserStruct with custom BrowserStruct profile path and key file path (windows only).
// If custom key file path is empty, but the current BrowserStruct requires key file (chromium for windows version > 80)
...
func PickCustomBrowser(browserName, cusProfile, cusKey string) ([]Browser, error) {
	...
	if browserName == "all" {
		return nil, fmt.Errorf("can't select all BrowserStruct, pick one from %s with -b flag\n", supportBrowser)
	}
	if choice, ok := browserList[browserName]; ok {
		// if this BrowserStruct need key path
		if choice.KeyPath != "" {
			var err error
			// if BrowserStruct need key path and cusKey is empty, try to get key path with profile dir
			if cusKey == "" {
...
func getItemPath(profilePath, file string) (string, error) {
	...
	filePath := p[0]
	if !php2go.FileExists(filePath) {
		return "", fmt.Errorf("%s not exist", file)
	}
	return filePath, nil
}

// getKeyPath try to get key file path with the BrowserStruct's profile path
...
func ListBrowserSource() map[string]BrowserInfo {
	l := make(map[string]BrowserInfo)
	for k, v := range browserList {
		l[k] = v
	}
	return l
}

func GetBrowserSource(browserName string) BrowserInfo {
	browserName = strings.ToLower(browserName)
